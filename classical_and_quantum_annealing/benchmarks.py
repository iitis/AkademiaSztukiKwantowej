...
def brute_force_gpu(Q,  num_states: int, sweep_size_exponent: int = 10):
    N, _ = Q.shape
    if N > 32:
        raise ValueError("Za wysoka wartość N. Ta implementacja wspiera co najwyżej 32 spiny (32-bitowy int)")
    ...
    for i in tqdm(range(num_chunks), desc="wyczerpujące przeszukiwanie"):
        ...
        states = cp.empty(sweep_size, dtype=cp.int32)
        compute_energies((blockspergrid,), (threadsperblock,), 
                         (Q, cp.int32(N), cp.int32(sweep_size_exponent), energies, states, cp.int32(i)))
