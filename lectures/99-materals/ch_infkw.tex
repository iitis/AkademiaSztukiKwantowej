...
Alice i~Bob (zwany dalej Bobem). Gracze mają do~dyspozycji
...
wykonuje Alice, drugi -- Bob, trzeci -- znowu Alice. Ruch polega
...
czy~reszką do~góry. Alice wygrywa, jeśli~moneta będzie w~pozycji
,,orzeł'', a~Bob -- jeśli~w~pozycji ,,reszka''.
...
kwantowej na~kubit -- za~pierwszym i~trzecim razem przez~Alice, a~za~drugim -- przez~Boba.
...
$$
Zatem Bob wygrywa z~prawdopodobieństwem $1$. Nie znaczy~to, że~Bob wygrywa
tę~grę zawsze, a tylko tyle, że~wygrywa w~przypadku takiego ciągu ruchów.
Jeśli ciąg ruchów będzie inny, np.~$\mat{X}$, $\mat{X}$, $\mat{I}$, może wygrać Alice.

Dajmy teraz Alice przewagę: ona będzie wiedzieć, że~grają na~kubicie, a~Bob nie~będzie tego świadom.
Zobaczmy teraz, co może zrobić Alice, jeżeli~wie, że~gra odbywa~się z~wykorzystaniem nie~monety, ale~kubitu.
...
Następnie Alice znowu nakłada bramkę Hadamarda:
\begin{equation*}
	\begin{split}
...
$$
W~efekcie Alice wygrywa, i~to wygrywa niezależnie od~działań Boba.
...
historyjka obrazkowa, która wyjaśnai jak Alice wygrała ze Bobem.
...
(zwane dalej Centrum), a~osobą próbującą przechwycić informację -- Alice.
...
tę wiadomość przed~Alice, dla~każdego bitu wiadomości wykonuje operację XOR
...
\begin{table}[h!]
	...
	\begin{tabular}{ccc}
		...
		1             & 1             & 0
	\end{tabular}
	\caption{Tabela argumentów i~wartości operacji $\text{wy}=\mathrm{XOR}(\text{we}_1, \text{we}_2)$}
	\label{tab:xor}
\end{table}
...
Wówczas w~procesie szyfrowania uzyskujemy szyfrogram ze~wzoru $s_i=\mathrm{XOR}(a_i, k_i)$ dla~każdego
$i=1,2,\ldots,n$. Centrum natomiast deszyfruje wiadomość, korzystając ze~wzoru
$b_i=\mathrm{XOR}(s_i, k_i)$. Łatwo można sprawdzić, że~dla~każdego $i=1,2,\ldots,n$ $a_i
...
Jeżeli~klucz jest całkowicie losowy, to~dla~Alice, która nie~zna klucza, szyfrogram
...
\textbf{Krok 1} Aleksandria wysyła sekwencję kubitów, z~których każdy jest w~losowo
i~niezależnie wybranym stanie: $\ket{0}$, $\ket{1}$,
$\ket{+}=\frac{\ket{0}+\ket{1}}{\sqrt{2}}$ lub~$\ket{-}=\frac{\ket{0}-\ket{1}}{\sqrt{2}}$.
\textbf{Krok 2} Centrum dokonuje -- również losowo i~niezależnie --
wyboru, który z~dwóch pomiarów -- $\mathcal{P}$ lub~$\mathcal{Q}$ -- przeprowadzi na~każdym z~kubitów z~osobna.
\textbf{Krok 3} Centrum zapisuje swój wybór i~wyniki pomiarów.
\textbf{Krok 4} Centrum wysyła do~Aleksandrii w~sposób jawny swój wybór pomiarów.
\textbf{Krok 5} Aleksandria przekazuje Centrum informację, które pomiary były dopasowane do stanów
nadanych kubitów.
\textbf{Krok 6} Aleksandria i~Centrum dzielą kubity na~dwie części: jedna zawiera te, które
...
\textbf{Krok 7} Aleksandria i~Centrum wybierają pewien wspólny
...
Zastanówmy~się, jakie próby ataku może podjąć Alice. Nie~może skopiować
...
podsłuchu jest bardzo łatwa do~zdemaskowania, ponieważ skoro Alice nie wie, jakie
kubity zostały nadane, nie~wie też, jaki pomiar powinna wybrać. Jeśli więc Alice wybierze
niewłaściwy pomiar, doprowadzi do~zmiany stanów kubitów, co --
po~wymianie klucza -- Aleksandria i~Centrum z~łatwością wykryją. Alice
...
proces ustalania klucza przez Aleksandrię i~Centrum, gdy~Alice
nie~podsłuchuje. Natomiast w~Tabeli~\ref{tab:bb84przykladzewą} pokazano,
co się~dzieje, gdy~Alice podsłuchuje komunikację kwantową.
...
\begin{table}[h]
	...
	\caption{
		...
		$A_1$ -- stany wysłane przez~Aleksandrię,
		$E_1$ -- pomiary wybrane przez~Alice,
		$E_2$ -- stany po~pomiarze Alice,
		$C_1$ -- pomiary wybrane przez~Centrum,
		$C_2$ -- stany po~pomiarze Centrum.
		Kolumny oznaczone na~szaro wyróżniają przypadki, dla~których stan kubitu
...
Rozważmy trzy osoby\footnote{Zwykle w protokole teleportacji myślimy o
	...
	i drugiej z jednym kubitem, która jest odbiorcą.} -- Bretta, Alice i
Roberta. Każde z nich ma do dyspozycji jeden kubit. Wyobraźmy sobie, że
kubit~Bretta jest w nieznanym stanie, a Alice chce przesłać stan kubitu
Bretta do Roberta. Alice może zbliżyć swój kubit do kubitu Bretta, jednak
...
niemożliwe. Natomiast jeżeli~Alice i~Robert współdzielą splątany stan
...
na~Rysunku~\ref{rys:teleportacja}. Pierwszy kubit to kubit Bretta, drugi należy do Alice, natomiast trzeci kubit ma
Robert. Pierwszy kubit jest w~stanie, który Alice chce przesłać Robertowi,
...
\begin{equation*}
	...
	\end{split}
\end{equation*}
Alice nakłada teraz bramkę~$\mat{CNOT}_1^2$ na~kubity swój i
Bretta\footnote{Jest to możliwe, jeśli zakładamy, że Alice może zbliżyć swój
...
Zauważmy, że~w~zależności od~wyniku pomiaru Alice stan kubitu Roberta jest
w tym momencie jednym z~czterech stanów: $\ket{\psi}$,
$\mat{Z}\ket{\psi}$, $\mat{X}\ket{\psi}$ lub~$\mat{X}\mat{Z}\ket{\psi}$.
Widać zatem, że~jeżeli~Robert wie, jaki wynik uzyskuje Alice, może~--
